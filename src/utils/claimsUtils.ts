 ...
 */
export const createClaimsTable = (claims: any): object => {
  let claimsObj = {};
  let index = 0;
...
*/
const populateClaim = (claim: string, value: string, description: string, index: number, claimsObject: any) => {
  let claimsArray = [];
  claimsArray[0] = claim;
  claimsArray[1] = value;
...
* @returns {boolean}
*/
export function compareIssuingPolicy(idTokenClaims: Record<string,string>, policyToCompare: string):boolean {
